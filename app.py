...
@st.cache_data(show_spinner=False)
def _cached_load_upload(data: bytes, filename: str) -> pd.DataFrame:
    class _BytesFile(io.BytesIO):
        def __init__(self, raw: bytes, name: str):
            super().__init__(raw)
            self.name = name

    return load_csv_from_upload(_BytesFile(data, filename))


def _load_raw(key, uploaded_file):
    if use_auto and auto_files_found.get(key):
        return _cached_load_csv(auto_files_found[key])
    if uploaded_file:
        data = uploaded_file.read()
        uploaded_file.seek(0)
        return _cached_load_upload(data, uploaded_file.name)
    return None
...
tab1, tab2, tab3, tab4 = st.tabs(["지도", "분석 결과", "상세 정보", "맞춤 대응 가이드"])
